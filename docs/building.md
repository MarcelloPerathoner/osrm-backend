...
about the build environment setup by Conan.  Usage: `source build/cmake-run.env`. It is
also handy on github CI, usage: `cat build/cmake-run.env >> $GIHUB_ENV`. It contains:
...
The file `build/cmake-run-env.sh`, generated by Cmake, can be sourced to setup an environment
in which the built binaries can be run. Usage: `source build/cmake-run-env.sh`. It contains:
