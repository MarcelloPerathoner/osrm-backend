...
make -C build/Release -j 16
cmake --install build/Release --component node_osrm -j 16
...
Conan puts `conan.env` and Cmake puts `cmake-run.env` there.  `.env` files should only
contain `KEY=VALUE` pairs so they can safely be sourced or piped into `$GITHUB_ENV`.

The file `conan.env` generated by Conan contains:
...
| `CONAN_LIBRARY_PATH`   | The list of directories containing the libraries.                           |

This file is only present in Conan builds.  After running Conan call Cmake like this:
`cmake -B $CONAN_BUILD_DIR --preset $CONAN_CMAKE_PRESET`


The file `cmake-run.env` generated by Cmake contains:
...
The file `cmake-run-env.sh`, generated by Cmake, can be sourced to setup an environment
in which the built binaries can be run.
