...
This file contains all information extracted from the github matrix. Its main use is for
piping into `$GITHUB_ENV` to make the environment available to the following job steps.

#### build/conan.env

This file contains information about the build environment setup by Conan. Usage:
`source build/conan.env`. It contains:
