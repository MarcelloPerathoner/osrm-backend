...
relations | Relations | Storage of relations to access relations, where `node` is a member.
...
result    |           | The output that you will modify.
relations | Relations | Storage of relations to access relations, where `way` is a member.
...
The `process_relation` function is called for every relation in an early stage, even
before @ref process_node and @ref process_way are being called.

Currently the only thing you can do here is to register @ref pedestrian_areas.

Argument  | Type      | Description
----------|-----------|------------------------------------------------------------------------
profile   | Profile   | The configuration table you returned in `setup`.
relation  | Relation  | The relation to process.
relations | Relations | Storage of relations to access relations, where `relation` is a member.
